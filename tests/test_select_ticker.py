...
class TestLivePortfolioPriority:
    """priority.txt is a fallback; the portfolio tracker is the source.

    Locally the sibling repo's user_portfolio.json is read live, so a new
    position is queued the moment it is bought. On CI the sibling repo is
    absent, so the committed priority.txt (regenerated by `make run`) stands
    in. The two must never be consulted together: a stale file would
    re-queue a sold position.
    """

    def tracker(self, path, held=(), watch=()):
        path.parent.mkdir(parents=True, exist_ok=True)
        path.write_text(json.dumps({
            "positions": [{"ticker": t, "lots": [{"quantity": 1}]}
                          for t in held],
            "watchlist": [{"ticker": t} for t in watch],
        }))
        return path

    def test_live_tracker_replaces_the_committed_file(self, st_repo,
                                                      monkeypatch):
        write_queue(st_repo, "priority.txt", ["STALE"])
        monkeypatch.setattr(st, "PORTFOLIO_JSON",
                            self.tracker(st_repo / "pt.json", held=["NEW"]))
        assert st.pick_new() == "NEW"
        give_reports(st_repo, "NEW")
        assert st.pick_new() is None          # STALE is never consulted

    def test_falls_back_to_the_file_without_a_tracker(self, st_repo,
                                                      monkeypatch):
        write_queue(st_repo, "priority.txt", ["FILE"])
        monkeypatch.setattr(st, "PORTFOLIO_JSON", st_repo / "absent.json")
        assert st.pick_new() == "FILE"

    def test_live_tickers_still_lead_the_exchange_sweeps(self, st_repo,
                                                         monkeypatch):
        write_queue(st_repo, "nzx.txt", ["AAA.NZ"])
        monkeypatch.setattr(st, "PORTFOLIO_JSON",
                            self.tracker(st_repo / "pt.json", watch=["W"]))
        assert st.pick_new() == "W"

    def test_live_tracker_works_without_a_priority_file(self, st_repo,
                                                        monkeypatch):
        # A fresh clone may not have queue/priority.txt yet.
        monkeypatch.setattr(st, "PORTFOLIO_JSON",
                            self.tracker(st_repo / "pt.json", held=["H"]))
        assert st.pick_new() == "H"

    def test_env_var_overrides_the_default_location(self, st_repo,
                                                    monkeypatch):
