...
Kept in `references/owner-fcf-adaptations.md`. **Read it before building when any of
these apply**: the filer reports in a currency other than the quote currency
(SMI.NZ, WISE.L, 9999.HK), a non-USD build (WACC must be built in the flows' own
currency), material leases (one lease treatment, never two), a minority interest
(BYD's ~34% BYDE), non-operating assets or excluded income (the matching-asset rule),
an SBC-zero market (house and street coincide; `band_position: null`), or a cyclical
where risk must be charged exactly once. Skipping it is worth four figures per share
on a Japanese supplier; reading it for a US SaaS name is ~2k tokens on every turn for
nothing.
